 ...
 */
@Component
@BusinessComponent(businName = "TwoBusinessComponent", bisinFriendlyName = "业务名称", version = "1.0", description =
        "desc",
...
//        QuarkComponentDescriptor quarkComponentDescriptor = QuarkComponentDescriptorFactory.getDescriptor
// (SuccessQuarkComponent.class);
        QuarkComponentDescriptor descriptor = QuarkComponentDescriptorFactory.getDescriptor(SuccessQuarkComponent5.class);
        descriptor.getOptions().setRetryLimitTimes(-1);

        Orchestration<FooFlow> orchestration =
                new DefaultOrchestration<FooFlow>()
                        .beginWith(QuarkComponentDescriptorFactory.getDescriptor(SuccessQuarkComponent.class))
                        .succeed()
                        .next(QuarkComponentDescriptorFactory.getDescriptor(SuccessQuarkComponent2.class))
                        .succeed()
                        .next(QuarkComponentDescriptorFactory.getDescriptor(SuccessQuarkComponent3.class))
                        .succeed()
                        .next(QuarkComponentDescriptorFactory.getDescriptor(SuccessQuarkComponent4.class))
                        .succeed()
                        .next(descriptor)
                        .succeed()
