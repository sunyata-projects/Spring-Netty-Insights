 ...
 */
public interface CompensationStateSynchronizer {
    public static CompensationStateSynchronizer getSynchronizer(Class<? extends AbstractBusinessComponent>
                                                                             businessComponentClazz) throws
            IllegalAccessException, InstantiationException {
        CompensationStateSynchronizer service = null;
        BusinessComponent annotation = businessComponentClazz.getAnnotation(BusinessComponent.class);
        try {
            service = ServiceLocator.getLocator().getService(annotation
                    .compensationSynchronizer());
        } catch (Exception ex) {
            
        }
        if (service == null) {
            service = new DefaultCompensationStateSynchronizer();
        }
        return service;
    }

    void sync(AbstractBusinessComponent businessComponent, BusinessContext businessContext, ProcessResult
