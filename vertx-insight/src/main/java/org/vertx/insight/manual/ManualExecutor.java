...
package org.vertx.insight.manual;
 ...
 */
public class ManualExecutor extends AbstractExecutor {
    Logger logger = LoggerFactory.getLogger(ManualExecutor.class);
...
//        BusinessLock lock = obtainBusinessLock(businessContext.getSerialNo());
        //lock.acquire();
        try {
            return execute(businessContext);
        } catch (Throwable ex) {
